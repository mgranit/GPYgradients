...
class MultioutputKern(CombinationKernel):
    ...
    def dK_dX(self, X, X2, dimX_0):
        ...
        slices2 = index_to_slices(X2[:,self.index_dim])
        target =  np.zeros((X.shape[0], X2.shape[0]))
        [[[[ target.__setitem__((slices[i][k],slices2[j][l]), self.covariance[i][j].dK_dX(X[slices[i][k],:],X2[slices2[j][l],:],dimX_0)) for k in range( len(slices[i]))] for l in range(len(slices2[j])) ] for i in range(len(slices))] for j in range(len(slices2))]
        return target
