...
class Prod(CombinationKernel):
    ...
    @Cache_this(limit=3, force_kwargs=['which_parts'])    
    def dK2_dXdX(self, X, X2, dim, dimX, which_parts=None):
        prod_sum = np.zeros((X.shape[0], X2.shape[0]))
        for combination1 in itertools.combinations(self.parts, len(self.parts) - 1):
            prod = reduce(np.multiply, [p.K(X, X2) for p in combination1])
            to_update1 = list(set(self.parts) - set(combination1))[0]
            prod_sum += prod*to_update1.dK2_dXdX(X, X2, dim, dimX)
            for combination2 in itertools.combinations(combination1, len(combination1) - 1):
                prod = reduce(np.multiply, [p.K(X, X2) for p in combination2]) if len(combination2) > 0 else np.ones(prod_sum.shape)
                to_update2 = list(set(combination1)-set(combination2))[0]
                prod_sum += prod*to_update1.dK_dX(X, X2, dim)*to_update2.dK_dX(X, X2, dimX)
        return prod_sum
    ...
    @Cache_this(limit=3, force_kwargs=['which_parts'])
    def dK3_dXdXdX2(self, X, X2, dim, dimX, dimX2, which_parts=None):
        prod_sum = np.zeros((X.shape[0], X2.shape[0]))
        for combination1 in itertools.combinations(self.parts, len(self.parts) - 1):
            prod = reduce(np.multiply, [p.K(X, X2) for p in combination1])
            to_update1 = list(set(self.parts) - set(combination1))[0]
            prod_sum += prod*to_update1.dK3_dXdXdX2(X, X2, dim, dimX, dimX2)
            for combination2 in itertools.combinations(combination1, len(combination1) - 1):
                prod = reduce(np.multiply, [p.K(X, X2) for p in combination2]) if len(combination2) > 0 else np.ones(prod_sum.shape)
                to_update2 = list(set(combination1)-set(combination2))[0]
                prod_sum += prod*to_update1.dK2_dXdX2(X, X2, dim, dimX2)*to_update2.dK_dX(X, X2, dimX)
                prod_sum += prod*to_update1.dK2_dXdX(X, X2, dim, dimX)*to_update2.dK_dX2(X, X2, dimX2)
                prod_sum += prod*to_update1.dK_dX(X, X2, dim)*to_update2.dK2_dXdX2(X, X2, dimX, dimX2)
                if len(self.parts) > 2:
                    for combination3 in itertools.combinations(combination2, len(combination2) - 1):
                        ...
                        prod_sum += prod*to_update1.dK_dX(X, X2, dim)*to_update2.dK_dX2(X, X2, dimX2)*to_update3.dK_dX(X, X2, dimX)
        return prod_sum

    @Cache_this(limit=3, force_kwargs=['which_parts'])
    def dK3_dXdXdX2diag(self, X, dim, dimX, which_parts=None):
        prod_sum = np.zeros(X.shape[0])
        for combination1 in itertools.combinations(self.parts, len(self.parts) - 1):
            prod = reduce(np.multiply, [p.Kdiag(X) for p in combination1])
            to_update1 = list(set(self.parts) - set(combination1))[0]
            prod_sum += prod*to_update1.dK3_dXdXdX2diag(X, dim, dimX)
            for combination2 in itertools.combinations(combination1, len(combination1) - 1):
                prod = reduce(np.multiply, [p.Kdiag(X) for p in combination2]) if len(combination2) > 0 else np.ones(prod_sum.shape)
                to_update2 = list(set(combination1)-set(combination2))[0]
                prod_sum += prod*to_update1.dK2_dXdX2diag(X, dim, dimX)*to_update2.dK_dXdiag(X, dimX)
                prod_sum += prod*to_update1.dK2_dXdXdiag(X, dim, dimX)*to_update2.dK_dX2diag(X, dimX)
                prod_sum += prod*to_update1.dK_dXdiag(X, dim)*to_update2.dK2_dXdX2diag(X, dimX, dimX)
                if len(self.parts) > 2:
                    for combination3 in itertools.combinations(combination2, len(combination2) - 1):
                        prod = reduce(np.multiply, [p.Kdiag(X) for p in combination3]) if len(combination3) > 0 else np.ones(prod_sum.shape)
                        to_update3 = list(set(combination2)-set(combination3))[0]
                        prod_sum += prod*to_update1.dK_dXdiag(X, dim)*to_update2.dK_dX2diag(X, dimX)*to_update3.dK_dXdiag(X, dimX)
        return prod_sum
